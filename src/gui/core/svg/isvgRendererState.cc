...
svg_status_t isvgRenderStateCreate( isvg_render_state_t **state ) {
    *state = (isvg_render_state_t*)malloc( sizeof( isvg_render_state_t ));
    if ( *state == NULL )
...
svg_status_t isvgRenderStateInitCopy( isvg_render_state_t *state, const isvg_render_state_t *other ) {
    ...
    *state = *other;
    ...
    if ( other->dash ) {
        state->dash = (double*)malloc( state->numDashes * sizeof( double ));
        if ( state->dash == NULL )
            return SVG_STATUS_NO_MEMORY;
...
isvg_render_state_t *isvgRenderStatePush( isvg_render_state_t *state ) {
    isvg_render_state_t *newst;
    isvgRenderStateCreate( &newst );
    if ( newst == NULL )
        return NULL;
    isvgRenderStateInitCopy( newst, state );
    newst->next = state;
    return newst;
}
