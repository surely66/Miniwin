...
static void ResetSMC(DWORD slot,BYTE*atr,UINT atr_len,NGL_SMCRESET_NOTIFY cbk){
    ...
    NGLOG_DEBUG("FR:%d, DR:%d, Fr:%d, Dr:%d, Fd: 372, Dd: 1\n", (int)FR, (int)DR, (int)Fr, (int)Dr);
	...
	NGLOG_DEBUG("set_pps=%d, TS0=0x%x\n", set_pps, (unsigned int)TS0);
  }else{
	//do not set pps if pps1_value = 0;
	set_pps = 0; 
  }	
  NGLOG_DEBUG("final pps1_value = 0x%x\n", pps1_value);
  retry_pps:
       if (set_pps){
           ...
           NGLOG_DEBUG("F:%d, D:%d, Fi:%d, Di:%d, Fd: 372, Dd: 1\n", (int)FI, (int)DI, (int)Fi, (int)Di);
           ...
           if (memcmp(tx_buffer, rx_buffer, 4) == 0){
    		...
    		if (T == 0) {
               ...
               if (AUI_RTN_SUCCESS != ret){
                    NGLOG_ERROR("aui_smc_param_set error, ret = %d\n", (int)ret);
                    ret_val = NGL_ERROR;
                    goto reset_thread_exit;
               }
               ...
               if (1 == DR){//DR = 1, can not set pps with the smallest value, do not need to try again
                   ...
                   pps1_value =  (FN << 4)| (DN & 0x0f);
                   NGLOG_DEBUG("retry pps1_value=0x%x\n", (unsigned int)pps1_value);
                   goto retry_pps;
               }
           }
       }//if set(pps)
...
static void SendCMD(DWORD slot_num,BYTE*cmd_buf,UINT cmd_size,NGL_SMCCOMMAND_NOTIFY cbk,void*userdata){
    ...
    NGLOG_DEBUG("slot num: %d, cmd_size=%d\n", (unsigned int)slot_num, cmd_size);
    
    cmd_head_size = 5;

    if (cmd_head_size == cmd_size){
        NGLOG_DEBUG("It is smart card read cmmmand!\n");    
        read_cmd = 1;
    }
    ...
    if (AUI_RTN_SUCCESS != ret){
        NGLOG_ERROR("aui_smc_raw_write error, ret = %d\n", (int)ret);
        goto send_thread_exit;
    }
    NGLOG_DEBUG("actlen: %d, cmd_data_size=%d\n", actlen, (int)cmd_data_size);
    ...
    if (cmd_data_size || read_cmd) {
        for (i = 0; i < 10; i++){
            ...
            if (AUI_RTN_SUCCESS != ret){
                NGLOG_ERROR("aui_smc_raw_read error, ret = %d\n", (int)ret);
                goto send_thread_exit;
            }
             
            if (recv_buffer[0] == 0x60) {
                NGLOG_DEBUG("NULL BYTE detected\n");
                continue;        
            } else if ((apdu_case_1 == 1) && (recv_buffer[0] == cmd_head[1])) {
                NGLOG_DEBUG("ACK received\n");
                continue;
            } else if ((apdu_case_1 == 1) && (recv_buffer[0] == (cmd_head[1] ^ 0xFF))) {
                NGLOG_DEBUG("ACK (INS ^ FF) received\n");
                continue;
            } else {
                NGLOG_DEBUG("SW1 detected\n");
                break;
            }
        }
        if (i >= 10) {
        ...
        if (((recv_buffer[0] >> 4) == 0x06) || ((recv_buffer[0] >> 4) == 0x09)){
            ...
            if (AUI_RTN_SUCCESS != ret){
                NGLOG_ERROR("aui_smc_raw_read SW2 error, ret = %d\n", (int)ret);
                goto send_thread_exit;
            }
    ...
    if (!read_cmd){ // send data
        ...
        for (i = 0; i < 10; i++){
            ...
            if (AUI_RTN_SUCCESS != ret){
                NGLOG_ERROR("aui_smc_raw_read SW1 error, ret = %d\n", (int)ret);
                goto send_thread_exit;
            }
        ...
        NGLOG_DEBUG("smart card read length=%d\n", (int)read_length);
        if (0 == read_length)
            read_length = 256;
...
static AUI_RTN_CODE *smc_init_cb(void *para)
{
    ...
    NGLOG_DEBUG("M3528 viaccess. running smc_init_cb");
    ...
    NGLOG_DEBUG("disable_pps: %d", pconfig->disable_pps);
...
{ 
   NGLOG_DEBUG("slot=%d pScState=%p state=%d",dwScSlot,pScState,((dwScSlot==0)?CARDS[dwScSlot].state:-1));
   CHECKSLOT(dwScSlot);
   if(NULL==pScState)
       return NGL_INVALID_PARA;
...
{ 
    ...
    if(CARDS[dwScSlot].locked>0)
        return NGL_ERROR;//RESOURCE_ALREADY_LOCKED;
    if(eSMCCARD_EXTRACTED==CARDS[dwScSlot].state)
        return NGL_INSERT_CARD_FIRST; 
...
{ 
   ...
   CHECKSLOT(dwScSlot);
   if(CARDS[dwScSlot].locked==0)
       return NGL_ERROR;//NGL_RESOURCE_NOT_LOCKED;
   CARDS[dwScSlot].locked--;
   if(eSMCCARD_EXTRACTED==CARDS[dwScSlot].state)
        return NGL_INSERT_CARD_FIRST; 
   return NGL_OK;
