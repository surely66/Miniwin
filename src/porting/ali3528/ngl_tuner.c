   ...
   while(true){
      ...
      for(i=0;i<NB_TUNERS;i++){
         ...
         while( timeout>0 && locked!=1 ){
             rc=aui_nim_is_lock(sTuners[i].hdl,&locked);
...
DWORD nglTunerInit(){
    ...
    for(i=0;i<NB_TUNERS;i++){
        ...
        NGLOG_DEBUG("init tuner %d",i);
        if(AUI_RTN_SUCCESS!=aui_find_dev_by_idx(AUI_MODULE_NIM,i,&sTuners[i].hdl)){
            NGLOG_DEBUG("aui_find_dev_by_idx reopen");
            ret = aui_nim_open(&sTuners[i].attr, &sTuners[i].hdl);
        }
        NGLOG_DEBUG("aui_nim_open=%d nim_id=%d type=%d hdl=%p",ret,i, sTuners[i].type,sTuners[i].hdl);
        if(sTuners[i].attr.en_dmod_type == AUI_NIM_QPSK && ret==0 ){
            ...
            NGLOG_DEBUG("aui_diseqc_oper =%d",ret);
        }
    }
...
static int dvbs_freq2inter(int freq,int polar,int *band,UINT LO)
{
    ...
    //NGLOG_DEBUG("*freq=%d lnb_mode=%d polar=%d",freq,lnb_mode,polar);
    if(LO > 0){
	if (lnb_mode == BAND_KU) {
            *band = (freq>11700)?AUI_NIM_HIGH_BAND:AUI_NIM_LOW_BAND;
            NGLOG_DEBUG("freq=%d LO=%d freq-LO=%d",freq,LO,freq-LO);
            ...
            *band = AUI_NIM_LOW_BAND;
            NGLOG_DEBUG("freq=%d LO=%d LO-freq=%d",freq,LO,LO-freq);
            return LO - freq;
    	}
    }
    if (lnb_mode == BAND_KU) {
...
DWORD  nglTunerLock(int tuneridx,NGLTunerParam*tp){
    ...
    case AUI_NIM_QPSK:	/* DVB-S */
        pc->ul_freq = dvbs_freq2inter(tp->u.s.frequency/1000,tp->u.s.polar, &high_band,0);
    ...
    default:
	NGLOG_DEBUG("%s %d -> nim type %d is wrong!", __FUNCTION__, __LINE__,tp->delivery_type);
	return NGL_ERROR;
    }
...
DWORD nglTunerSetLNB(int tuneridx,int polarity){
    ...
    gpio_power.io = AUI_GPIO_O_DIR;
    gpio_power.uc_dev_idx = 96;//control  H/V
    ret= aui_gpio_open(&gpio_power, &lnbhdl);
    //ret=aui_nim_get_lnb_gpio(sTuners[tuneridx].hdl,&lnbhdr);
    if(AUI_RTN_SUCCESS==ret)
        ret=aui_gpio_set_value(lnbhdl,polarity);
    aui_gpio_close(lnbhdl);
...
DWORD nglSendDiseqcCommand(int tuneridx,BYTE*Command,BYTE cmdlen,BYTE diseqc_version){
    ...
    NGLOG_VERBOSE("hdl=%p aui_diseqc_oper=%d  cmdlen=%d",hdl,rc,cmdlen);
