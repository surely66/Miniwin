...
BOOL diseqc_format_diseqc10_command(TUNER_DISEQC_COMMAND_ENUM eCommand,BYTE* pbCommand, PTUNER_DISEQC_BODY_STRUCT pstDiseqcCommand)
{
	BOOL   tRet = FALSE;
           		...
           		case TUNER_DISEQC_CMD_SETSOB:
            		...
            		case TUNER_DISEQC_CMD_WRITE_N0:
	                	...
	                	pbCommand[TUNER_DISEQC_DATA_BYTE_0_OFFSET] = 0xf0 | ((((pstDiseqcCommand->eDiSEqC10 - 1) * 4) & 0x0f)
	                                                               | ((pstDiseqcCommand->ePolarization == TUNER_POL_VERTICAL) ?  0 : 2) 
...
BOOL diseqc_format_diseqc11_uncommit_command(TUNER_DISEQC_COMMAND_ENUM eCommand,BYTE* pbCommand, PTUNER_DISEQC_BODY_STRUCT pstDiseqcCommand)
{
	BOOL                        tRet = FALSE;
...
BOOL diseqc_format_diseqc11_commit_command(TUNER_DISEQC_COMMAND_ENUM eCommand,BYTE* pbCommand, PTUNER_DISEQC_BODY_STRUCT pstDiseqcCommand)
{
	BOOL                        tRet = FALSE;
...
BOOL diseqc_format_diseqc12_commit_command(TUNER_DISEQC_COMMAND_ENUM eCommand, BYTE* pbCommand, TUNER_DISEQC_MOTOR_BODY_STRUCT Option)
{
	BOOL                        tRet = TRUE;
    	...
    	if (pbCommand){
        	...
        	switch (eCommand){
	            	...
	            	case TUNER_DISEQC_CMD_DRIVE_WEST:
	            		pbCommand[TUNER_DISEQC_DATA_BYTE_0_OFFSET]  = 0 == Option.Body.MotorCmd.bCmd1 ? Option.Body.MotorCmd.bCmd0 : (0xff - (Option.Body.MotorCmd.bCmd0 - 1));
                		break;
...
BOOL diseqc_set_diseqc10(int eIndexOfTuner, TUNER_DISEQC10_ENUM eDiSEqC10, TUNER_TP_POLARIZATION_ENUM ePolar, TUNER_TONE_ENUM e22K)
{
	BOOL tRet = FALSE;
...
BOOL diseqc_set_diseqc11(int eIndexOfTuner, TUNER_DISEQC11_ENUM eDiSEqC11, TUNER_TP_POLARIZATION_ENUM ePolar, TUNER_TONE_ENUM e22K)
{
	BOOL tRet = FALSE;
	BYTE pbCommand[TUNER_DISEQC_CMD_LENGTH];
	TUNER_DISEQC_BODY_STRUCT stDiseqcCommand;
...
USHORT diseqc_caculate_motor_angular(USHORT usSatLongtitude, USHORT usLocalLongtitude, USHORT usLocalLatitude)
{
	...
	if (TRUE){
	    	...
	    	G15 =
	        (DOUBLE) sqrtf(DISEQC_USALS_G15_CONS -
	                       (DISEQC_USALS_G15_CONS * cosf((PI * ABS(temp_var) / DISEQC_USALS_CONS_4))));
