...
public:
     ...
     USHORT getStreamId(){return extTableId();}
     int getPrograms(PROGRAMMAP*maps,bool nit=false);
     int getPMTPID(USHORT program_number);
};
...
public:
     BYTE*getProgramDescriptors(INT&len){
...
public:
     ...
     USHORT getStreamId(){return extTableId();}
     USHORT getNetId(){return data[8]<<8|data[9];}
     int getServices(DVBService*services,bool own=true);
};
...
public:
    ...
    int getEvents(EVENT*events,bool own=true);
...
public:
   USHORT tsid;
   USHORT netid;
   bool getDelivery(NGLTunerParam*tp);
   int getServices(SERVICELOCATOR*svc);
...
public:
   ...
   BYTE*getNetworkDescriptors(int&len){
       len=((data[8]&0x0F)<<8)|data[9];
       return data+10;
   }
   virtual int getName(char*name,const char*lan);
   int getStreams(STREAM*ts,bool own);
   int getLCN(SERVICELOCATOR*svc,USHORT*lcn);//get lcn from first descriptor loop
   int matchServices(SERVICELOCATOR*svc,int size);//only used for LCNV1 in first descriptor
};
...
public:
   ...
   BYTE*getBouquetDescriptors(int&len){return getNetworkDescriptors(len);} 
   virtual int getName(char*name,const char*lan)override;
};

class TDT:public PSITable{
public:
   TDT(const BYTE*dt):PSITable(dt){}
   void getTime(INT&mjd,INT&utc){
      mjd=data[3]<<8|data[4];
...
public:
   TOT(const BYTE*dt):TDT(dt){}
   BYTE*getDescriptors(int&len);
   int getTimeOffset(TimeOffset*tms,int size);
};
