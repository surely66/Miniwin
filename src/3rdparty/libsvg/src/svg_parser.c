...
static svg_status_t
_svg_parser_new_container_element (svg_parser_t 	*parser,
			       	   svg_dom_node_t	*node,
			       	   svg_element_t	**container_element,
				   svg_element_type_t 	type);

static svg_status_t
_svg_parser_new_svg_group_element (svg_parser_t 	*parser, 
			       	   svg_dom_node_t	*node,
...
_svg_parser_parse_defs (svg_parser_t		*parser,
		       svg_dom_node_t		*node,
		       svg_element_t		**defs_element);

static svg_status_t
_svg_parser_parse_use (svg_parser_t		*parser,
		       svg_dom_node_t		*node,
		       svg_element_t		**use_element);
...
_svg_parser_parse_line (svg_parser_t		*parser,
			svg_dom_node_t		*node,
			svg_element_t		**line_element);

static svg_status_t
_svg_parser_parse_rect (svg_parser_t		*parser,
			svg_dom_node_t		*node,
			svg_element_t		**rect_element);
...
static svg_status_t
_svg_parser_parse_clip_path (svg_parser_t		*parser,
			     svg_dom_node_t		*node,
			     svg_element_t		**clip_path_element);

static svg_status_t
_svg_parser_parse_mask (svg_parser_t		*parser,
			svg_dom_node_t		*node,
			svg_element_t		**mask_element);

static svg_status_t
_svg_parser_parse_marker (svg_parser_t		*parser,
			  svg_dom_node_t	*node,
...
static svg_status_t
_svg_parser_parse_gradient_end (svg_parser_t	*parser,
				svg_element_t	*gradient_element);

static svg_status_t
_svg_parser_parse_pattern_end (svg_parser_t	*parser,
			       svg_element_t	*pattern_element);
			       ...
			       const xmlChar	**xml_atts)
{
    ...
    svg_dom_node_t *node;
    svg_status_t status;
    svg_nsname_t *nsname = NULL;
    svg_nsatts_t *nsatts = NULL;
    long line_number;
    
    line_number = _svg_parser_get_line_number (parser);

    status = _svg_parser_copy_xml_name (&parser->dom, &nsname, xml_name);
    if (status)
	goto error;
    
    status = _svg_parser_copy_xml_attributes (&parser->dom, &nsatts, nsname->ns_index, xml_atts);
    if (status)
	goto error;
    
    status = _svg_dom_start_element (&parser->dom, nsname, nsatts, &node);
    if (status)
	goto error;
...
error:
    _svg_parser_set_node_error (parser, line_number, nsname, status);

    if (nsname != NULL)
    	_svg_dom_destroy_nsname (nsname);
    if (nsatts != NULL)
	_svg_dom_destroy_nsatts (nsatts);
			     ...
			     const xmlChar	*xml_name)
{
    ...
    if (ch_node != NULL && ch_node->ch != NULL &&
    		_svg_compare_nsname_svg (nsname, "style") == 0) {
    ...
    status = _svg_dom_end_element (&parser->dom, nsname);
    if (status)
	goto error;
    nsname = NULL;
			    ...
			    int			len)
{
    ...
    if (_svg_compare_nsname_svg (element_node->nsname, "style") != 0) {
	cb = NULL;
	for (i = 0; i < SVG_ARRAY_SIZE (SVG_PARSER_MAP); i++) {
	    if (_svg_compare_nsname_svg (element_node->nsname, SVG_PARSER_MAP[i].name) == 0) {
...
error:
    _svg_parser_set_node_error (parser, line_number, element_node->nsname, status);
					     ...
					     const xmlChar	**xml_atts)
{
    ...
    status = _svg_parser_copy_xml_name (dom, &nsname, xml_name);
    if (status)
	goto error;
    
    status = _svg_parser_copy_xml_attributes (dom, &nsatts, nsname->ns_index, xml_atts);
    if (status)
	goto error;
    
    status = _svg_dom_start_element (dom, nsname, nsatts, &node);
    if (status)
	goto error;
    /* node now has ownership */
    nsname = NULL;
...
{
    if (node_nsname != NULL)
	_svg_set_error_element_name (parser->svg, node_nsname);
    else
			       ...
			       svg_status_t	error_status)
{
	...
	_svg_parser_set_node_error (parser, node->line_number, node->nsname, error_status);
    }
    else {
	parent_node = _svg_dom_get_parent (node);
	if (parent_node != NULL)
	    _svg_parser_set_node_error (parser, node->line_number, parent_node->nsname, error_status);
	else
...
_svg_parser_process_dom (svg_parser_t *parser)
{
    ...
    if (_svg_compare_nsname_svg (parser->dom.root_node->nsname, "svg") != 0) {
	if (parser->dom.root_node->nsname->ns_index == NO_NAMESPACE_INDEX &&
	    strcmp (parser->dom.root_node->nsname->local_name, "svg") == 0)
	{
	...
	return _svg_parser_return_node_error (parser, def_element->element->node, SVG_STATUS_UNKNOWN_REF_ELEMENT);
    }
...
_svg_parser_process_dom_element_node (svg_parser_t *parser, svg_dom_node_t *node)
{
	...
	status = _svg_get_css_stylesheet_declarations (&node->css_properties, 
	...
	_svg_attribute_get_string (node->nsatts, "style", &style_str, NULL);
	if (style_str) {
	    status = _svg_parse_inline_css (&node->inline_css_properties, 
    ...
    cb = NULL;
    for (i = 0; i < SVG_ARRAY_SIZE (SVG_PARSER_MAP); i++) {
	if (_svg_compare_nsname_svg (node->nsname, SVG_PARSER_MAP[i].name) == 0) {
	    cb = &SVG_PARSER_MAP[i].cb;
	    break;
	}
    }
...
_svg_parser_process_dom_character_node (svg_parser_t *parser, svg_dom_node_t *node)
{
    ...
    status = (parser->state->cb->parse_characters) (parser, node, ch_data + ch_data_offset, ch_data_len);
				      ...
				      svg_parser_deferred_element_t 	*def_element)
{
	...
	if (status)
	    if (status != SVG_STATUS_DUPLICATE_ELEMENT_ID || !def_element->element->node->is_deep_clone)
		goto end;
    }
				   ...
				   svg_element_t	**svg_group_element)
{
    ...
    status = _svg_element_create (svg_group_element,
			      ...
			      svg_element_type_t 	type)
{
    ...
    status = _svg_element_create (&new_element, 
    				  type,
				  node,
				  parser->state->container_element,
				  parser->svg);
    if (status)
	return status;
...
static svg_status_t
_svg_parser_new_virtual_leaf_element (svg_parser_t 		*parser,
			      	      svg_dom_node_t		*node,
				      svg_element_t 		**child_element,
				      svg_element_type_t	type)
{
    ...
    status = _svg_create_virtual_element (&new_element, 
    					  type,
					  node,
					  parser->state->container_element,
					  parser->svg);
...
_svg_parser_parse_symbol (svg_parser_t		*parser,
			  ...
			  svg_element_t		**symbol_element)
{
    return _svg_parser_new_container_element (parser, node, symbol_element, SVG_ELEMENT_TYPE_SYMBOL);
}
			...
			svg_element_t		**text_element)
{
    ...
    status = _svg_parser_new_container_element (parser,
    						node,
					    	text_element,
				   ...
				   size_t		len)
{
    ...
    status = _svg_parser_new_virtual_leaf_element (parser,
    					   	   node->parent,
						   &tspan_element,
						   SVG_ELEMENT_TYPE_TSPAN);
    if (status)
