...
static svg_status_t
_svg_parse_uri (svg_t *svg, svg_uri_t *uri);

static svg_status_t
_svg_read_cb (void *closure, const char *buffer, size_t buffer_size);

static svg_status_t
_svg_resolve_element_ref (svg_t 	*svg,
			  svg_t		*from_svg,
	    ...
	    void		*target_closure)
{
    ...
    if (svg->trace != NULL) {
	status = _svg_trace_push_target_engine (svg->trace,
						svg->document_uri,
						target_engine, 
						target_closure);
	if (status)
...
printf("%s:%d doc_uri=%p base_uri=%p url=%s\r\n",__FILE__,__LINE__,document_uri,base_uri,url);
    status = _svg_create_uri_ref_url (url, base_uri, document_uri, &abs_uri_ref);
printf("%s:%d status=%d\r\n",__FILE__,__LINE__,status);
    if (status)
	return status;

printf("%s:%d  abs_uri_ref=%p\r\n",__FILE__,__LINE__,abs_uri_ref);    
    if (abs_uri_ref->element_id == NULL) {
	_svg_destroy_uri_ref (abs_uri_ref);
	return SVG_STATUS_MISSING_REF_ELEMENT_ID;
    }
...
_svg_parse_uri (svg_t *svg, svg_uri_t *uri)
{
	...
	status = _svg_uri_clone (&abs_uri, uri);
	if (status)
	    return status;
    }

    if (!svg->user_set_base_uri) {
	status = _svg_uri_clone (&new_base_uri, abs_uri);
	if (status) {
...
_svg_read_cb (void *closure, const char *buffer, size_t buffer_size)
{
	...
	if (data->peek_bytes[0] == gz_magic[0] &&
	    data->peek_bytes[1] == gz_magic[1])
	{
	...
	do {
	    ...
	    status = svg_parse_chunk (data->svg, (char*)data->buffer, SVG_PARSE_BUFFER_SIZE - data->strm.avail_out);
	    if (status)
		return status;
	} 
			  ...
			  svg_element_t **element)
{
	...
	if (svg->parent != NULL && from_svg != svg->parent) {
	    ...
	    if (status) {
		if (status != SVGINT_STATUS_UNKNOWN_DOCUMENT)
		    return status;
	    }
	    else
		return SVG_STATUS_SUCCESS;
	...
	status = _svg_parse_uri (new_child, abs_uri_ref->uri);
	if (status) {
