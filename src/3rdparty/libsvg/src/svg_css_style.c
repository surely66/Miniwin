...
from the Inkscape (http://www.inkscape.org/) implementation in 
inkscape/src/sp-style-elem.cpp */ 
...
static svg_status_t
_svg_parse_declaration (svg_css_declaration_t *declaration, const char *start_decl, const char *end_decl);

static svg_status_t
_svg_parse_declaration_str (char **str, const char *start_str, const char *end_str);

static svg_status_t
_svg_trim_str (const char *start_str, const char *end_str, const char **new_start_ref, const char **new_end_ref);


svg_status_t 
_svg_create_css_stylesheet (svg_css_stylesheet_t **stylesheet)
{
    *stylesheet = (svg_css_stylesheet_t*)malloc (sizeof(svg_css_stylesheet_t));
    if (*stylesheet == NULL)
		       ...
		       size_t 			count)
{
    ...
    (void)buffer;
    (void)count;
    printf("todo:svg_parse_css_buffer:%d[%s]\r\n",count,buffer);    
    /* not yet implemented */
...
svg_status_t 
_svg_parse_inline_css (svg_css_declaration_t	**declarations_ref, 
		       ...
		       size_t 			count)
{
    ...
    printf("_svg_parse_inline_css data=%s\r\n",data); 
	...
	*num_declarations_ref = 0;
    ...
    declarations = (svg_css_declaration_t*)malloc (num_declarations * sizeof(svg_css_declaration_t));
    if (declarations == NULL)
	return SVG_STATUS_NO_MEMORY;
...
static svg_status_t 
_svg_parse_css (CRParser 		*cr_parser, 
		svg_css_stylesheet_t 	*stylesheet,
		svg_uri_t 		*base_uri);

static svg_status_t
_svg_create_css_parser (svg_css_parser_t 	**parser, 
			svg_css_stylesheet_t 	*stylesheet,
			svg_uri_t		*base_uri);
...
static void
_svg_cr_start_selector (CRDocHandler 	*a_handler,
                       	CRSelector 	*a_sel_list);

static void
_svg_cr_end_selector (CRDocHandler 	*a_handler,
                      CRSelector 	*a_sel_list);

static void
_svg_cr_start_font_face (CRDocHandler		*a_handler,
...
static void 
_svg_cr_import_style (CRDocHandler 	*a_this,
		      GList 		*a_media_list,
		      CRString 		*a_uri,
...
static void
_svg_cr_property (CRDocHandler 	*a_handler,
		  CRString 	*a_name,
		  CRTerm 	*a_value, 
...
static void
_svg_cr_inline_property (CRDocHandler	*a_handler,
			 CRString 	*a_name,
			 CRTerm 	*a_value, 
			 gboolean 	a_important);
...
_svg_create_css_stylesheet (svg_css_stylesheet_t **stylesheet)
{
    *stylesheet = malloc (sizeof (svg_css_stylesheet_t));
    if (*stylesheet == NULL)
	return SVG_STATUS_NO_MEMORY;
    
    memset (*stylesheet, 0, sizeof(svg_css_stylesheet_t));
    
    (*stylesheet)->cr_stylesheet = cr_stylesheet_new (NULL);
				    ...
				    svg_uri_t 			*uri)
{
    ...
    status = _svg_resource_read_stream_to_buffer (uri, &buffer, &buffer_size, MAX_CSS_RESOURCE_SIZE);
    if (status)
	return status;
		       ...
		       size_t 			count)
{
	...
	for (i = 0; i < num_declarations; i++) {
	    ...
	    declarations[i].name = strdup ((const char*)cr_string_peek_raw_str (cr_decl->property));
	    if (declarations[i].name == NULL) {
		status = SVG_STATUS_NO_MEMORY;
		goto fail;
	    }
...
svg_status_t
_svg_get_css_stylesheet_declarations (svg_css_declaration_t	**declarations_ref, 
				      int 			*num_declarations_ref, 
				      svg_dom_node_t 		*node, 
				      svg_css_stylesheet_t 	*stylesheet)
{
	...
	while (cur_pair != NULL)
	{
	    ...
	    if (cr_decl != NULL) 
	    {
		declarations[index].name = strdup ((const char*)cr_string_peek_raw_str (cr_decl->property));
...
_svg_create_css_parser (svg_css_parser_t 	**parser, 
			...
			svg_uri_t 		*base_uri)
{
    *parser = malloc (sizeof (svg_css_parser_t));
    if (*parser == NULL)
	return SVG_STATUS_NO_MEMORY;
    
    memset (*parser, 0, sizeof(svg_css_parser_t));
                      ...
                      CRSelector 	*a_sel_list)
{
    ...
    if (parser->stmt_type == NORMAL_RULESET_STMT &&
        ...
        parser->stylesheet->cr_stylesheet->statements = cr_statement_append (
		parser->stylesheet->cr_stylesheet->statements, ruleset);
    }
...
static void 
_svg_cr_import_style (CRDocHandler 	*a_handler,
		      GList 		*a_media_list,
		      CRString 		*a_uri,
		      CRString 		*a_uri_default_ns,
		      CRParsingLocation	*a_location)
...
_svg_create_inline_css_parser (svg_inline_css_parser_t **parser)
{
    *parser = malloc (sizeof (svg_inline_css_parser_t));
    if (*parser == NULL)
	return SVG_STATUS_NO_MEMORY;
    
    memset (*parser, 0, sizeof(svg_inline_css_parser_t));
