...
static svg_status_t
_svg_marker_render_group_start (svg_element_t			*element,
				      svg_render_engine_t	*engine,
				      void			*closure);

static svg_status_t
_svg_marker_render_group_end (svg_element_t		*element,
			      svg_render_engine_t	*engine,
			      void			*closure);
...
_svg_marker_init (svg_marker_int_t *marker, svg_element_t *marker_element)
{
    ...
    status = _svg_create_element_ref (&marker->ext_marker.element_ref, marker_element);
    if (status)
	return status;

    _svg_length_init_unit (&marker->ref_x, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
    _svg_length_init_unit (&marker->ref_y, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);

    _svg_length_init_unit (&marker->width, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
		       ...
		       svg_marker_int_t	*other)
{
    ...
    status = _svg_create_element_ref (&ext_marker->element_ref, marker_element);
    if (status)
	return status;
			      ...
			      const svg_nsatts_t	*attributes)
{
	...
	return _svg_element_return_property_error (marker_element, "markerUnits", SVG_STATUS_INVALID_VALUE);
	...
	if (status)
	    return _svg_element_return_property_error (marker_element, "preserveAspectRatio", status);
    }
		    ...
		    void		*closure)
{
	...
	status = _svg_view_box_transform (&marker->view_box, marker_width, marker_height, &transform);
	if (status)
	    return status;
	
	status = _svg_transform_coordinate (&transform,
					    ref_x, ref_y,
					    &viewport_ref_x, &viewport_ref_y);
	if (status)
	    return status;
	...
	_svg_length_init_unit (&viewport_x, -ref_x, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
	_svg_length_init_unit (&viewport_y, -ref_y, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
    }
    ...
    status = _svg_engine_end_group (engine, closure, _svg_style_get_opacity (&element->node->style));
    if (status)
	return status;
...
svg_status_t
svg_marker_render (const svg_marker_t	*ext_marker,
		   double		stroke_width,
		   svg_render_engine_t	*engine,
		   void			*closure)
{
    svg_marker_int_t *non_const_marker = (svg_marker_int_t*)&ext_marker->element_ref->element->e.marker;
				...
				void			*closure)
{
	...
	status = _svg_transform_init_scale (&units_transform, marker->stroke_width, marker->stroke_width);
	if (status)
	    return status;
