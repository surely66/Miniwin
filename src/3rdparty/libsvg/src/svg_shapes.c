...
static svg_status_t
_svg_shape_end_render (svg_element_t		*element,
		       svg_render_engine_t	*engine,
		       void			*closure);

static svg_status_t
_svg_shape_begin_render (svg_element_t	*element,
			 svg_render_engine_t	*engine,
			 void			*closure);
			      ...
			      const svg_nsatts_t	*attributes)
{
	...
	return _svg_element_return_property_error (rect_element, "height", SVG_STATUS_INVALID_VALUE);
		  ...
		  void			*closure)
{
    ...
    status = _svg_engine_render_line (engine, closure,
				    &line->x1, &line->y1,
				    &line->x2, &line->y2);
    if (status)
	return status;
		  ...
		  void			*closure)
{
    ...
    status = _svg_engine_render_rect (engine, closure,
				    &rect->x, &rect->y,
...
{
    return _svg_engine_support_render_circle (engine) &&
    	element->e.circle.r.value != 0;
}
		    ...
		    void		*closure)
{
    ...
    status = _svg_engine_render_circle (engine, closure,
				      &circle->cx, &circle->cy,
				      &circle->r);
    if (status)
	return status;
		     ...
		     void			*closure)
{
    ...
    status = _svg_engine_render_ellipse (engine, closure,
