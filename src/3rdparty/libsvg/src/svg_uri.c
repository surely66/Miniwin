...
_svg_uri_create_absolute (svg_uri_t *base_uri, svg_uri_t *uri, svg_uri_t **abs_uri)
{
	...
	status = _svg_uri_clone (&new_uri, uri);
	if (status)
	    return status;
    }
    else {
	...
	result = uriAddBaseUriA (&temp_new_uri->uri, &uri->uri, base_uri);
	if (result) {
...
printf("%s:%d temp_new_uri=%s uri_str=%s status=%d\r\n",__FILE__,__LINE__,temp_new_uri->uri_str,uri_str,status);  
	if (status)
	    goto fail;
...
_svg_uri_is_file (const svg_uri_t *uri)
{
    return !_svg_uri_is_data_scheme (uri) && 
        ...
        strncmp (uri->uri.scheme.first, "file", uri->uri.scheme.afterLast - uri->uri.scheme.first) == 0;
}
...
{
#if defined(_WIN32)
    return ((uri_str[0] >= 'a' && uri_str[0] <= 'z') || 
    		(uri_str[0] >= 'A' && uri_str[0] <= 'Z')) &&
    	uri_str[1] == ':';
#else
    return uri_str[0] == '/';
#endif
...
_svg_uri_to_string (const svg_uri_t *uri, char **uri_str)
{
	...
	result = uriToStringCharsRequiredA (&uri->uri, &uri_str_len);
        printf("_svg_uri_to_string uri_str_len=%d:%x result=%d\r\n",uri_str_len,uri_str_len,result);
	if (result)
...
svg_status_t
svg_decode_data_scheme_uri (const char *uri_str,
		     	    char **media_type_ref,
			    unsigned char **buffer_ref,
			    size_t *buffer_size_ref)
{
    svg_uri_t *uri;
    ...
    status = _svg_resource_read_stream_to_buffer (uri,
						  &buffer,
						  &buffer_size,
						  -1);
    if (status) {
...
_svg_uri_init (const char *uri_str, svg_uri_t *uri)
{
    ...
    printf("_svg_uri_init uri_str=[%s]  uri->uri_str=%p\r\n",uri_str, uri->uri_str);
    if (uri->uri_str == NULL)
	return SVG_STATUS_NO_MEMORY;
