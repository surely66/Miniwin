...
static svgint_status_t
_svg_attribute_find (const svg_nsatts_t	*attributes,
		     const char		*name,
		     const svg_nsatt_t	**attribute);

static svgint_status_t
_svg_attribute_find_ns (const svg_nsatts_t	*attributes,
			int			ns_index,
			const char		*name,
			const svg_nsatt_t	**attribute);
		     ...
		     const svg_nsatt_t	**attribute)
{
	...
	if (_svg_compare_nsname_svg (&attributes->atts[i].name, name) == 0) {
			...
			const svg_nsatt_t	**attribute)
{
    ...
    for (i = 0; i < attributes->num; i++) {
	if (_svg_compare_nsname_2 (&attributes->atts[i].name, ns_index, name) == 0) {
	    *attribute = &attributes->atts[i];
	    return SVG_STATUS_SUCCESS;
	}
    }
