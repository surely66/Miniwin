...
svg_status_t
_svg_gradient_stop_apply_attributes (svg_element_t	*stop_element,
			...
			svg_gradient_type_t 	type)
{
	...
	_svg_length_init_unit (&ext_gradient->u.linear.x1, 0, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_HORIZONTAL);
	_svg_length_init_unit (&ext_gradient->u.linear.y1, 0, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_VERTICAL);
	_svg_length_init_unit (&ext_gradient->u.linear.x2, 100, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_HORIZONTAL);
	_svg_length_init_unit (&ext_gradient->u.linear.y2, 0, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_VERTICAL);
    } else {
	_svg_length_init_unit (&ext_gradient->u.radial.cx, 50, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_HORIZONTAL);
	_svg_length_init_unit (&ext_gradient->u.radial.cy, 50, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_VERTICAL);
	_svg_length_init_unit (&ext_gradient->u.radial.fx, 50, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_HORIZONTAL);
	_svg_length_init_unit (&ext_gradient->u.radial.fy, 50, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_VERTICAL);
	_svg_length_init_unit (&ext_gradient->u.radial.r, 50, SVG_LENGTH_UNIT_PCT, SVG_LENGTH_ORIENTATION_OTHER);
    }
			...
			svg_gradient_stop_t	*stop)
{
    ...
    if (ext_gradient->num_stops == 0 || stop->offset >= ext_gradient->stops[ext_gradient->num_stops - 1].offset)
				...
				const svg_nsatts_t	*attributes)
{
    ...
    svg_gradient_t *ext_prototype = NULL;
	...
	status = _svg_resolve_element_href (gradient_element->svg,
                                        ...
                                        gradient_element->node->base_uri,
                                        href,
                                        &ref);
	if (status) {
	...
	if (ref->type != SVG_ELEMENT_TYPE_GRADIENT)
	    return _svg_element_return_property_error (gradient_element, "href", SVG_STATUS_WRONG_REF_ELEMENT_TYPE);
	
	prototype = &ref->e.gradient;
	ext_prototype = &ref->e.gradient.ext_gradient;
	...
	if (strcmp (str, "userSpaceOnUse") == 0)
	    ...
	    return _svg_element_return_property_error (gradient_element, "gradientUnits", SVG_STATUS_INVALID_VALUE);
    }    
	...
	if (status)
	    return _svg_element_return_property_error (gradient_element, "gradientTransform", status);
    } 
	...
	if (strcmp (str, "pad") == 0)
	    ...
	    return _svg_element_return_property_error (gradient_element, "spreadMethod", SVG_STATUS_INVALID_VALUE);
    }
