...
_svg_clip_path_init (svg_clip_path_int_t *clip_path, svg_element_t *clip_path_element)
{
    ...
    status = _svg_create_element_ref (&clip_path->ext_clip_path.element_ref, clip_path_element);
    if (status)
	return status;
				 ...
				 const svg_nsatts_t	*attributes)
{
    ...
    status = _svg_attribute_get_string (attributes, "clipPathUnits", &str, "userSpaceOnUse");
	...
	return _svg_element_return_property_error (clip_path_element, "clipPathUnits", SVG_STATUS_INVALID_VALUE);
