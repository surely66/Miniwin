...
static svg_status_t
_svg_style_parse_marker_common (const svg_marker_t **marker, svg_dom_node_t *node, svg_t *svg, const char *str);
...
static svg_status_t
_svg_style_apply_css_properties (svg_dom_node_t	*node, 
				 svg_t		*svg,
				 int		do_important);
...
static svg_status_t
_svg_style_compute_font_size (svg_dom_node_t 		*node,
			      svg_render_engine_t	*engine,
			      void			*closure);

static svg_status_t
_svg_style_return_property_error (svg_t		*svg,
...
_svg_style_init_empty (svg_style_t *style)
{
    ...
    _svg_length_init_unit (&style->font_size, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
    _svg_length_init_unit (&style->stroke_dash_offset, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_OTHER);
    _svg_length_init_unit (&style->stroke_width, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_OTHER);
    _svg_length_init_unit (&style->clip_rect[0], 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
    _svg_length_init_unit (&style->clip_rect[1], 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
    _svg_length_init_unit (&style->clip_rect[2], 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
    _svg_length_init_unit (&style->clip_rect[3], 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
			     ...
			     svg_t		*svg)
{
	...
	_svg_attribute_get_string (node->nsatts, map->name, &str, NULL);
		   ...
		   void			*closure)
{
	...
	status = _svg_engine_set_stroke_dash_array (engine, closure, style->stroke_dash_array, style->num_dashes);
	if (status)
	    return status;
    }
					  ...
					  void			*closure)
{
    ...
    if (style->overflow_hidden) {
	status = _svg_engine_viewport_clipping_path (engine, closure, 
						     &style->clip_rect[0],
						     &style->clip_rect[1], 
						     &style->clip_rect[2], 
						     &style->clip_rect[3]);
...
_svg_style_parse_fill_paint (svg_dom_node_t *node, svg_t *svg, const char *str)
{
	...
	status = _svg_paint_init (&node->style.fill_paint, svg, node->document_uri, node->base_uri, str);
	if (status)
	    return status;
    }
...
_svg_style_parse_stroke_dash_array (svg_dom_node_t *node, svg_t *svg, const char *str)
{
	...
	if (node->parent->style.num_dashes > 0) {
		...
		return SVG_STATUS_NO_MEMORY;
	    memcpy (new_dash_array, node->parent->style.stroke_dash_array, node->parent->style.num_dashes * sizeof(double));
	}
	else
	    new_dash_array = NULL;
	...
	status = _svg_str_parse_all_csv_doubles (str, &node->style.stroke_dash_array, &node->style.num_dashes, &end);
	if (status)
	    return status;
	...
	if (node->style.num_dashes % 2) {
	    ...
	    new_dash_array = realloc(node->style.stroke_dash_array, node->style.num_dashes * sizeof(double));
	    if (new_dash_array == NULL)
		return SVG_STATUS_NO_MEMORY;
	    node->style.stroke_dash_array = new_dash_array;
...
_svg_style_parse_stroke_paint (svg_dom_node_t *node, svg_t *svg, const char *str)
{
	...
	status = _svg_paint_init (&node->style.stroke_paint, svg, node->document_uri, node->base_uri, str);
	if (status)
	    return status;
    }
				 ...
				 int		do_important)
{
	...
	for (i = 0; i < SVG_ARRAY_SIZE(SVG_STYLE_PARSE_MAP); i++) {
		...
		status = (SVG_STYLE_PARSE_MAP[i].parse) (node, svg, node->inline_css_properties[j].value);
		break;
	    }
	}
	if (status == SVG_STATUS_NO_MEMORY)
...
_svg_style_compute_colors (svg_dom_node_t *node)
{
    ...
    if (style->fill_paint.type == SVG_PAINT_TYPE_COLOR && style->fill_paint.p.color.is_current_color)
	style->fill_paint.p.color = style->color;

    if (style->stroke_paint.type == SVG_PAINT_TYPE_COLOR && style->stroke_paint.p.color.is_current_color)
			      ...
			      void			*closure)
{
    ...
    if (!(style->flags & SVG_STYLE_FLAG_FONT_SIZE) && 
	node->parent != NULL &&
	node->parent->style.computed_font_size >= 0)
    {
    ...
    status = _svg_engine_measure_font_size (engine, closure,
    					    style->font_family,
					    parent_font_size,
					    &style->font_size, 
					    &style->computed_font_size);
    if (status)
	return status;
