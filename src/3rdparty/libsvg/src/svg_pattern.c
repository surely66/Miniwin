...
_svg_pattern_init (svg_pattern_int_t *pattern, svg_element_t *pattern_element)
{
    ...
    status = _svg_create_element_ref (&pattern->ext_pattern.element_ref, pattern_element);
    if (status)
	return status;
    ...
    _svg_length_init_unit (&ext_pattern->x, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
    _svg_length_init_unit (&ext_pattern->y, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
    _svg_length_init_unit (&ext_pattern->width, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
    _svg_length_init_unit (&ext_pattern->height, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
			...
			svg_pattern_int_t 	*other)
{
    ...
    status = _svg_create_element_ref (&ext_pattern->element_ref, pattern_element);
    if (status)
	return status;
			       ...
			       const svg_nsatts_t	*attributes)
{
	...
	if (prototype_element->type != SVG_ELEMENT_TYPE_PATTERN)
	    return _svg_element_return_property_error (pattern_element, "href", SVG_STATUS_INVALID_VALUE);
	...
	if (strcmp (str, "userSpaceOnUse") == 0)
	    ...
	    return _svg_element_return_property_error (pattern_element, "patternUnits", SVG_STATUS_INVALID_VALUE);
    }
	...
	if (strcmp (str, "userSpaceOnUse") == 0)
	    ...
	    return _svg_element_return_property_error (pattern_element, "patternContentUnits", SVG_STATUS_INVALID_VALUE);
    }
    ...
    if (h_len.value < 0)
	return _svg_element_return_property_error (pattern_element, "width", SVG_STATUS_INVALID_VALUE);
    if (status != SVG_STATUS_ATTRIBUTE_NOT_FOUND)
    ...
    if (v_len.value < 0)
	return _svg_element_return_property_error (pattern_element, "height", SVG_STATUS_INVALID_VALUE);
    if (status != SVG_STATUS_ATTRIBUTE_NOT_FOUND)
	...
	if (status)
	    return _svg_element_return_property_error (pattern_element, "preserveAspectRatio", status);
    }
		     ...
		     void			*closure)
{
    ...
    _svg_length_init_unit (&zero_horiz_length, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
    _svg_length_init_unit (&zero_vert_length, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
