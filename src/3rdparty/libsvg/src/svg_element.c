			     ...
			     svg_t		*svg)
{
    ...
    status = _svg_create_dom_node (&virtual_node, ELEMENT_NODE_TYPE);
    if (status)
	return status;
    ...
    status = _svg_element_create (element, type, virtual_node, parent, svg);
    if (status) {
	_svg_destroy_dom_node (virtual_node);
	return status;
    }
		    ...
		    void		*target_closure)
{
	...
	status = _svg_trace_push_target_engine (element->svg->trace,
						element->node->document_uri,
						target_engine, 
						target_closure);
	if (status)
	    return status;
	...
	_svg_trace_set_position_info (element->svg->trace, 
				      element->node->nsname->local_name,
	...
	_svg_set_error_element_name (element->svg, element->node->nsname);
    }
...
_svg_element_apply_properties (svg_element_t *element)
{
    ...
    status = _svg_transform_apply_attributes (element, &element->transform, element->node->nsatts);
    if (status)
	return status;

    _svg_attribute_get_string (element->node->nsatts, "id", &id, NULL);
    if (id == NULL)
	_svg_attribute_get_string_ns (element->node->nsatts, XML_NAMESPACE_INDEX, "id", &id, NULL);
    if (id != NULL)
	element->id = strdup (id);

    _svg_attribute_get_string (element->node->nsatts, "class", &klass, NULL);
    if (klass != NULL)
	element->klass = strdup (klass);

    switch (element->type) {
    case SVG_ELEMENT_TYPE_SVG_GROUP:
	status = _svg_svg_group_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_GROUP:
	status = _svg_group_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_SYMBOL:
	status = _svg_symbol_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_USE:
	status = _svg_use_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_PATH:
	status = _svg_path_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_DEFS:
	break;
    case SVG_ELEMENT_TYPE_RECT:
	status = _svg_rect_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_CIRCLE:
	status = _svg_circle_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_ELLIPSE:
	status = _svg_ellipse_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_LINE:
	status = _svg_line_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_TEXT:
	status = _svg_text_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_TSPAN:
	status = _svg_tspan_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_IMAGE:
	status = _svg_image_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_GRADIENT:
	status = _svg_gradient_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_GRADIENT_STOP:
	status = _svg_gradient_stop_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_PATTERN:
	status = _svg_pattern_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_CLIP_PATH:
	status = _svg_clip_path_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_MASK:
	status = _svg_mask_apply_attributes (element, element->node->nsatts);
	break;
    case SVG_ELEMENT_TYPE_MARKER:
	status = _svg_marker_apply_attributes (element, element->node->nsatts);
	break;
    }
