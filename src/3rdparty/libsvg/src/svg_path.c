...
{
    return _svg_engine_support_render_path (engine) &&
    	element->e.path.ext_path != NULL;
}
			    ...
			    const svg_nsatts_t	*attributes)
{
	...
	while (*p) {
	    ...
	    if (status)
		return _svg_element_return_property_error (path_element, "points", SVG_STATUS_PARSE_ERROR);
...
int
svg_center_point_param_arc (double x1, double y1,
			    const svg_path_arc_to_t *e_arc,
