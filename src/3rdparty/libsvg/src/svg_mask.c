...
_svg_mask_init (svg_mask_int_t *mask, svg_element_t *mask_element)
{
    ...
    status = _svg_create_element_ref (&ext_mask->element_ref, mask_element);
    if (status)
	return status;
    ...
    _svg_length_init_unit (&ext_mask->width, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_HORIZONTAL);
    _svg_length_init_unit (&ext_mask->height, 0, SVG_LENGTH_UNIT_PX, SVG_LENGTH_ORIENTATION_VERTICAL);
			    ...
			    const svg_nsatts_t	*attributes)
{
	...
	return _svg_element_return_property_error (mask_element, "maskUnits", SVG_STATUS_INVALID_VALUE);
	...
	return _svg_element_return_property_error (mask_element, "maskContentUnits", SVG_STATUS_INVALID_VALUE);
