/* svg_dom.c: Simple SVG XML DOM implementation for CSS
...
static svg_status_t
_svg_attach_new_node (svg_dom_t *root, svg_dom_node_t *new_node);

static svg_status_t
_svg_append_child_node (svg_dom_node_t *parent, svg_dom_node_t *new_child);

static int svg_utf8_code_len (const char *utf8, size_t utf8_len);

static unsigned int svg_next_utf8_code_point (const char *utf8, size_t utf8_len, int *code_len);
...
_svg_dom_init (svg_dom_t *dom)
{
	...
	status = _svg_init_uri_map_entry (&dom->ns_map[i], 
	    SVG_KNOWN_NS_MAP[i].uri, SVG_KNOWN_NS_MAP[i].index);
	if (status)
	    return status;
    }
...
{
    svg_nsname_t *new_nsname;
    svg_status_t status;
    
    new_nsname = (svg_nsname_t*)malloc (sizeof(svg_nsname_t));
    if (new_nsname == NULL)
	return SVG_STATUS_NO_MEMORY;

    memset (new_nsname, 0, sizeof(svg_nsname_t));
    
    status = _svg_dom_init_nsname (new_nsname, ns_index, local_name);
    if (status) {
	_svg_dom_destroy_nsname (new_nsname);
	return status;
    }
...
_svg_dom_init_nsname (svg_nsname_t 	*nsname, 
		      ...
		      const char 	*local_name)
{
    nsname->local_name = strdup (local_name);
    if (nsname->local_name == NULL)
	return SVG_STATUS_NO_MEMORY;
    
    nsname->ns_index = ns_index;
...
{
    if (nsname->local_name != NULL) {
	free (nsname->local_name);
	nsname->local_name = NULL;
    }
}
...
_svg_dom_create_nsatts (svg_nsatts_t 	**nsatts,
			int		num_nsatts)
{
    svg_nsatts_t *new_nsatts;
    
    new_nsatts = (svg_nsatts_t*)malloc (sizeof(svg_nsatts_t));
    if (new_nsatts == NULL)
	return SVG_STATUS_NO_MEMORY;
    
    memset (new_nsatts, 0, sizeof(svg_nsatts_t));
    
    if (num_nsatts > 0) {
	new_nsatts->atts = (svg_nsatt_t*)malloc (num_nsatts * sizeof(svg_nsatt_t));
	if (new_nsatts->atts == NULL) {
	    _svg_dom_destroy_nsatts (new_nsatts);
	    return SVG_STATUS_NO_MEMORY;
	}
	new_nsatts->num = num_nsatts;
	
	memset (new_nsatts->atts, 0, num_nsatts * sizeof(svg_nsatt_t));
    }
		     ...
		     const char		*value)
{
    ...
    status = _svg_dom_init_nsname (&nsatt->name, ns_index, local_name);
    if (status)
	return status;
    
    nsatt->value = NULL;
    if (value != NULL) {
	nsatt->value = strdup (value);
	if (nsatt->value == NULL)
	    return SVG_STATUS_NO_MEMORY;
    }
...
{
    _svg_dom_deinit_nsname (&nsatt->name);
...
_svg_dom_destroy_nsatts (svg_nsatts_t *nsatts)
{
    ...
    if (nsatts->atts != NULL) {
	for (i = 0; i < nsatts->num; i++)
	    _svg_dom_deinit_nsatt (&nsatts->atts[i]);
...
{
    return _svg_compare_nsname_2 (left, SVG_NAMESPACE_INDEX, svg_local_name);
}
...
_svg_destroy_dom_node (svg_dom_node_t *node)
{
    ...
    if (node->nsname != NULL && node->own_nsname)
	_svg_dom_destroy_nsname (node->nsname);
    if (node->nsatts != NULL && node->own_nsatts)
	_svg_dom_destroy_nsatts (node->nsatts);
	...
	if (node->inline_css_properties != NULL)
	    _svg_destroy_css_declarations (node->inline_css_properties, node->num_inline_css_properties);
    }
			...
			svg_dom_node_t 	**node)
{
    ...
    status = _svg_create_dom_node (&new_node, ELEMENT_NODE_TYPE);
    if (status)
	return status;

    /* assign and take ownership */
    new_node->nsname = nsname;
    new_node->own_nsname = 1;
    new_node->nsatts = nsatts;
    new_node->own_nsatts = 1;
    
    status = _svg_attach_new_node (dom, new_node);
    if (status)
	goto fail;

    _svg_attribute_get_string_ns (nsatts, XML_NAMESPACE_INDEX, "space", &str, "");
    if (strcmp (str, "preserve") == 0)
	new_node->preserve_space = 1;
    else if (strcmp (str, "default") == 0)
	new_node->preserve_space = 0;
    
    _svg_attribute_get_string_ns (nsatts, XML_NAMESPACE_INDEX, "base", &str, NULL);
    if (str != NULL) {
...
fail:
    if (new_node != NULL) {
	new_node->nsname = NULL;
	new_node->nsatts = NULL;
		      ...
		      svg_nsname_t	*nsname)
{
    ...
    _svg_dom_destroy_nsname (nsname);
		     ...
		     svg_dom_node_t 	**node) 
{
    ...
    if (dom->current_node->type != CHARACTER_NODE_TYPE) {
	status = _svg_create_dom_node (&character_node, CHARACTER_NODE_TYPE);
	if (status)
	    return status;
    }
    else
			  ...
			  svg_dom_node_t *from_node)
{
	...
	status = _svg_create_dom_node (&new_node, CHARACTER_NODE_TYPE);
	if (status)
	    return status;
	...
	new_node->own_nsname = 0;
	new_node->own_nsatts = 0;
	...
	while (from_child != NULL) {
	    status = _svg_dom_deep_clone_node (&new_child_node, new_node, from_child);
	    if (status)
		goto fail;
			  ...
			  const char 		*local_name)
{
    ...
    status = _svg_dom_create_nsname (&new_nsname, ns_index, local_name);
    if (status)
	return status;
			  ...
			  const char 		*value)
{
	...
	status = _svg_dom_create_nsatts (&new_nsatts, 1); 
	if (status)
	    return status;
	
	status = _svg_dom_init_nsatt (&new_nsatts->atts[0], ns_index, local_name, value);
	if (status) {
	    _svg_dom_destroy_nsatts (new_nsatts);
	    return status;
	}
	
	node->nsatts = new_nsatts;
	node->own_nsatts = 1;
    }
    else {
	if (!node->own_nsatts) {
	    status = _svg_dom_create_nsatts (&new_nsatts, node->nsatts->num); 
	    if (status)
		return status;
	    
	    for (i = 0; i < node->nsatts->num; i++) {
		status = _svg_dom_init_nsatt (&new_nsatts->atts[i], 
					      node->nsatts->atts[i].name.ns_index, 
					      node->nsatts->atts[i].name.local_name, 
					      node->nsatts->atts[i].value);
		if (status) {
		    _svg_dom_destroy_nsatts (new_nsatts);
		    return status;
		}
	...
	index = -1;
	for (i = 0; i < node->nsatts->num; i++) {
	    if (_svg_compare_nsname_2 (&node->nsatts->atts[i].name, ns_index, local_name) == 0) {
		index = i;
		break;
	    }
	}
	...
	if (index < 0) {
	    ...
	    new_atts = realloc (node->nsatts->atts, (node->nsatts->num + 1) * sizeof(svg_nsatt_t));
	    if (new_atts == NULL)
		return SVG_STATUS_NO_MEMORY;
	    
	    node->nsatts->atts = new_atts;
	    
	    status = _svg_dom_init_nsatt (&node->nsatts->atts[node->nsatts->num], ns_index, local_name, value);
	    if (status)
		return status;
	    ...
	    if (value == NULL) {
		if (node->nsatts->num == 1) {
		    _svg_dom_destroy_nsatts (node->nsatts);
		    node->nsatts = NULL;
		}
		else {
		    _svg_dom_deinit_nsatt (&node->nsatts->atts[index]);
		    if (index != node->nsatts->num - 1)
			memcpy (&node->nsatts->atts[index], &node->nsatts->atts[node->nsatts->num - 1], sizeof(svg_nsatt_t));
		...
		free (node->nsatts->atts[index].value);
		node->nsatts->atts[index].value = new_value;
	    }
	}
    }
...
_svg_dom_get_local_attr_value (svg_dom_node_t *node, const char *local_name)
{
    ...
    if (node->nsatts != NULL)
	for (i = 0; i < node->nsatts->num; i++)
	    if (strcmp (node->nsatts->atts[i].name.local_name, local_name) == 0)
		return node->nsatts->atts[i].value;
			       ...
			       size_t		*ch_data_len_ref)
{
	...
	*ch_data_len_ref = 0;
	...
	for (src = node->ch, dst = ch_data, i = 0; i < node->len; i += code_len) {
	    code_point = svg_next_utf8_code_point (src, node->len - i, &code_len);
	    if (code_len == 0) {
		free (ch_data);
...
static int svg_utf8_code_len (const char *utf8, size_t utf8_len)
{
    ...
    else if (((unsigned char)utf8[0] & 0xf0) == 0xe0)
    {
	    ...
	    ((unsigned char)utf8[1] & 0xc0) == 0x80 &&
            ((unsigned char)utf8[2] & 0xc0) == 0x80)
        {
            return 3;
        }
    }
    else if (((unsigned char)utf8[0] & 0xf8) == 0xf0)
    {
	    ...
	    ((unsigned char)utf8[1] & 0xc0) == 0x80 &&
            ((unsigned char)utf8[2] & 0xc0) == 0x80 &&
            ((unsigned char)utf8[3] & 0xc0) == 0x80)
        {
            return 4;
        }
    }
