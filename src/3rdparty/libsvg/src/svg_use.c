			   ...
			   const svg_nsatts_t	*attributes)
{
    ...
    status = _svg_resolve_element_href (use_element->svg,
                                        use_element->node->document_uri, 
                                        use_element->node->base_uri,
                                        href,
                                        &ref_element);
    if (status) {
    ...
    status = _svg_dom_deep_clone_node (&cloned_node, use_element->node, ref_element->node);
    if (status)
	return status;
    ...
    {
	status = _svg_dom_node_set_nsname (cloned_node, SVG_NAMESPACE_INDEX, "svg");
	if (status)
	    return status;
	
	status = _svg_dom_node_set_nsattr (cloned_node, SVG_NAMESPACE_INDEX, "x", "0");
	if (status)
	    return status;
	status = _svg_dom_node_set_nsattr (cloned_node, SVG_NAMESPACE_INDEX, "y", "0");
	if (status)
	    return status;
	status = _svg_dom_node_set_nsattr (cloned_node, SVG_NAMESPACE_INDEX, "width", width_str);
	if (status)
	    return status;
	status = _svg_dom_node_set_nsattr (cloned_node, SVG_NAMESPACE_INDEX, "height", height_str);
	if (status)
	    return status;
    }
