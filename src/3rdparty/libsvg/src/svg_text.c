...
static svg_status_t
_svg_prepare_tspan (svg_element_t 	*element,
		    svg_render_engine_t	*engine,
		    void 		*closure);

static svg_status_t
_svg_text_width (svg_text_t		*text,
		 double			*width,
		 int			start_element,
		 svg_render_engine_t	*engine,
		 void			*closure);
		  ...
		  void			*closure)
{
	...
	status = _svg_engine_begin_element (engine, closure, tspan_element->id, tspan_element->klass);
	if (status)
	    return status;
