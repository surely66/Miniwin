			   ...
			   const xmlChar 	*xml_name)
{
	...
	*sep = g_ns_separator;
    ...
    status = _svg_dom_create_nsname (nsname, ns_index, local_name);
    if (status)
	return status;
				 ...
				 const xmlChar	**xml_atts)
{
    ...
    int ns_index = 0;
    int i;
    svg_nsatts_t *new_nsatts = NULL;
    int num_nsatts;
    svg_status_t status;
    
    num_nsatts = 0;
    while (xml_atts[num_nsatts] != NULL)
	num_nsatts++;
    num_nsatts = (num_nsatts + 1) / 2;
	...
	*nsatts = NULL;
    ...
    status = _svg_dom_create_nsatts (&new_nsatts, num_nsatts); 
    if (status)
	return status;
    
    for (i = 0; i < new_nsatts->num; i++) {
	...
	status = _svg_dom_init_nsatt (&new_nsatts->atts[i], ns_index, local_name, value);
	if (status) {
	    _svg_dom_destroy_nsatts (new_nsatts);
	    return status;
	}
