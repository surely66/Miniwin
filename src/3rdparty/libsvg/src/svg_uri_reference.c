		     ...
		     svg_uri_ref_t 	**uri_ref)
{
    ...
    printf("%s:%d uri_refstr=%s\r\n",__FILE__,__LINE__,uri_ref_str);
	...
	*fragment_start = '\0';
    ...
    if (fragment_start != NULL)
	*fragment_start = '#';
    printf("%s:%d status =%d uri_refstr=%s\r\n",__FILE__,__LINE__,status,uri_ref_str);
    if (status)
	goto fail;
    
    if (base_uri != NULL && _svg_uri_is_relative (new_uri_ref->uri)) {
    printf("%s:%d _svg_uri_is_empty_path=%d\r\n",__FILE__,__LINE__,_svg_uri_is_empty_path(new_uri_ref->uri));
...
_svg_create_uri_ref_url (const char 	*attribute, 
			 ...
			 svg_uri_ref_t 	**uri_ref)
{
    printf("%s _svg_create_uri_ref_url attribute=%s base_uri=%p document_uri=%p\r\n",__FUNCTION__,attribute,base_uri,document_uri);
