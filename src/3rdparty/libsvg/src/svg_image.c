...
static svg_status_t
_svg_image_read_image_file (const char		*filename,
		       	    char	 	**data,
			    unsigned int	*width,
			    unsigned int	*height);

static svg_status_t
_svg_image_read_image_buffer (const char	*buffer,
			      size_t		buffer_size,
		       	      char	 	**data,
			      unsigned int	*width,
			      unsigned int	*height);
...
static svg_status_t
_svg_image_read_image (svg_image_source_t	*source,
		       char	 		**data,
		       unsigned int		*width,
		       unsigned int		*height);

static svg_status_t
_svg_image_read_png (svg_image_source_t	*source,
		     char	 	**data,
		     unsigned int	*width,
		     unsigned int	*height);

static svg_status_t
_svg_image_read_jpeg (svg_image_source_t	*source,
		      char	 		**data,
		      unsigned int		*width,
		      unsigned int		*height);
		      ...
		      svg_image_t *other)
{
    ...
    *image = *other;
    ...
    if (other->uri != NULL) {
	status = _svg_uri_clone (&image->uri, other->uri);
	if (status)
	    return status;
    }
			     ...
			     const svg_nsatts_t	*attributes)
{
	...
	return _svg_element_return_property_error (image_element, "width", SVG_STATUS_INVALID_VALUE);
	...
	return _svg_element_return_property_error (image_element, "height", SVG_STATUS_INVALID_VALUE);
		   ...
		   void			*closure)
{
    ...
    status = _svg_engine_begin_element (engine, closure,
					element->id,
					element->klass);
    if (status)
	return status;
    ...
    status = _svg_engine_render_image (engine, closure,
				       uri_str,
				       image->index,
				       &image->view_box_template,
				       &image->x,
				       &image->y,
				       &image->width,
				       &image->height);
...
svg_status_t
svg_get_bgra_image (const char 		*uri_str,
		    ...
		    unsigned int 	*image_data_height)
{
	...
	status = _svg_image_read_image_file (filename,
					     (char**)image_data,
					     image_data_width,
					     image_data_height);
...
svg_status_t
svg_get_bgra_image_from_buffer (const char		*media_type,
...
{
    return _svg_image_read_image_buffer ((char*)buffer, buffer_size, (char**)data, data_width, data_height);
}

svg_status_t
svg_get_image_filename (const char 	*uri_str,
		    	char 		**filename,
			int 		*is_temp_copy)
{
    svg_status_t status;
...
svg_status_t
svg_get_relative_uri (svg_t		*svg,
		      const char 	*uri_str,
		      char		**rel_uri_str)
{
    svg_uri_t *uri;
		       ...
		       unsigned int		*height)
{
    ...
    status = _svg_image_read_png (source,
				  data,
				  width,
				  height);
    if (status == 0)
    ...
    status = _svg_image_read_jpeg (source,
				   data,
				   width,
				   height);
    if (status == 0)
...
#if 0//def PNG_READ_EXPAND_SUPPORTED
