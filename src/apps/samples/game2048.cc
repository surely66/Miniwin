...
public:
   GameWindow(int x,int y,int w,int h):Window(x,y,w,h){
      tfscore=new TextField("0",200,40);
      addChildView(tfscore)->setPos(800,50);
   }
   void getColor(uint8_t value, char *color, size_t length);
   void initBoard(uint16_t size);
   void drawBoard(GraphContext&canvas);
...
void GameWindow::drawBoard(GraphContext&canvas) {
	uint8_t x,y;
...
bool GameWindow::onKeyRelease(KeyEvent&k){
    ...
    case NGL_KEY_RIGHT: success=moveRight();break;
