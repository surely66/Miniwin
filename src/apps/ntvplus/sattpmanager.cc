...
TPManagerWindow::TPManagerWindow(int x,int y,int w,int h)
    ...
    satlst->setFlag(Attr::ATTR_BORDER);
    tplst->setBgColor(0xFF000000);
    tplst->setFgColor(0xFFFFFFFF);
    tplst->setFlag(Attr::ATTR_BORDER);
    addChildView(satlst);
    addChildView(tplst);
    ...
    lb=new TextField("Signal Quality",150,30);
    lb->setPos(650,555);
    addChildView(lb)->setBgColor(0xFF000000).setFgColor(0xFFFFFFFF);
...
bool TPManagerWindow::onKeyRelease(KeyEvent&k){
    bool satmd=satlst->hasFlag(Attr::ATTR_FOCUSED);
    switch(k.getKeyCode()){
    case NGL_KEY_LEFT:
    case NGL_KEY_RIGHT:
